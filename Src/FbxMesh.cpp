...
void CFbxMesh::RenderSkin(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	if (animStatus.animNum != animStatus.animNumOld) {
		animStatus.animFrame = 0;
		animStatus.animFloatFrame = 0.0f;
		animStatus.animNumOld = animStatus.animNum;
	}
	...
	if (animStatus.playAnim) {
		animStatus.animFloatFrame += animStatus.animSpeed;
		animStatus.animFrame = (int)animStatus.animFloatFrame;
...
void CFbxMesh::RenderToonStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxStaticMeshToon_VS, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxStaticMeshToon_PS, NULL, 0);
...
{
	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxSkinMeshToon_VS, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxSkinMeshToon_PS, NULL, 0);

	// トゥーンテクスチャをシェーダーに渡す    // -- 2018.10.2
	SetShaderTexture(_T("PS"), 4, &m_pD3D->m_pToonTexture);
