...
// トゥーンシェーディング用
	SAFE_RELEASE(m_pFbxStaticMesh_VertexLayout);
	SAFE_RELEASE(m_pFbxStaticMeshToon_VS);
	SAFE_RELEASE(m_pFbxStaticMeshToon_PS);

	// Fbxモデル　スキンメッシュ用のシェーダー
	// トゥーンシェーディング用
	SAFE_RELEASE(m_pFbxSkinMesh_VertexLayout);
	SAFE_RELEASE(m_pFbxSkinMeshToon_VS);
	SAFE_RELEASE(m_pFbxSkinMeshToon_PS);
...
HRESULT CShader::InitShader()
{
	...
	//  Fbxスタティック・スキンメッシュ(トゥーンシェーディング)用のシェーダー作成
	InitShaderFbxToon();
...
HRESULT CShader::InitShaderFbxToon()
{
	...
	MakeShader(_T("VS"), _T("FbxStaticMeshToon_VS.cso"), (void**)&m_pFbxStaticMeshToon_VS);
	MakeShader(_T("PS"), _T("FbxStaticMeshToon_PS.cso"), (void**)&m_pFbxStaticMeshToon_PS);
	...
	// 頂点インプットレイアウトを定義
	// バーテックスシェーダ・ピクセルシェーダの作成
	MakeShader(_T("VS"), _T("FbxSkinMeshToon_VS.cso"), (void**)&m_pFbxSkinMeshToon_VS);
	MakeShader(_T("PS"), _T("FbxSkinMeshToon_PS.cso"), (void**)&m_pFbxSkinMeshToon_PS);
