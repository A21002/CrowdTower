...
CBaseProc::~CBaseProc()
{
	SAFE_DELETE(m_pMesh);                       // メッシュの削除  // -- 2022.2.16
...
void CBaseProc::SetHoldItem(DWORD weaponId)
{
	// 各武器ごとに遷移
	switch (weaponId) {
	case ATTACK_ALLOW:
		...
		m_HoldItem.m_nPosBone = 13;	// 剣の根元位置のボーン番号
	...
	case ATTACK_SWORD:
		m_HoldItem.m_nHoldObjNo = 9;	// 手に持つ武器のオブジェ番号(透明な剣)
		m_HoldItem.m_nPosMesh = 0;		// 剣の根元位置のメッシュ番号
		m_HoldItem.m_nPosBone = 12;	// 剣の根元位置のボーン番号
	...
	case ATTACK_GUN:
		m_HoldItem.m_nHoldObjNo = 10;	// 手に持つ武器のオブジェ番号(透明な剣)
		m_HoldItem.m_nPosMesh = 0;		// 剣の根元位置のメッシュ番号
		m_HoldItem.m_nPosBone = 12;	// 剣の根元位置のボーン番号
		m_HoldItem.m_nOffsetPos = VECTOR3(-0.1f, 0.25f, 0.06f);  // 手に持つ武器の位置オフセット
		m_HoldItem.m_nOffsetRot = VECTOR3(90.0f, 1.0f, 108.0f);  // 手に持つ武器の角度オフセット
		break;

	case ATTACK_AXE:
		m_HoldItem.m_nHoldObjNo = 11;	// 手に持つ武器のオブジェ番号(透明な剣)
		m_HoldItem.m_nPosMesh = 0;		// 剣の根元位置のメッシュ番号
		m_HoldItem.m_nPosBone = 12;	// 剣の根元位置のボーン番号
		m_HoldItem.m_nOffsetPos = VECTOR3(0.0f, 0.14f, 0.04f);  // 手に持つ武器の位置オフセット
		m_HoldItem.m_nOffsetRot = VECTOR3(0.0f, -90.0f, 0.0f);  // 手に持つ武器の角度オフセット
		break;
...
void CBaseProc::ResetHoldItem()
{
	m_HoldItem.m_nHoldObjNo = 0;	// 手に持つ武器のオブジェ番号(透明な剣)
	m_HoldItem.m_nPosMesh = 0;		// 剣の根元位置のメッシュ番号
	m_HoldItem.m_nPosBone = 0;	// 剣の根元位置のボーン番号
...
BOOL   CBaseObj::Hitcheck(CBaseObj* pOtherObj, CBBox* pBBoxAtack)
{
	...
	// バウンディングボックス同士による判定
	if ((m_bActive && m_dwStatus != DAMAGE && m_dwStatus != DEAD && m_dwStatus != FLASH && this != pOtherObj) &&
		(pOtherObj->m_bActive && pOtherObj->m_dwStatus != DAMAGE && pOtherObj->m_dwStatus != DEAD && pOtherObj->m_dwStatus != FLASH) &&
...
BOOL CBaseObj::TargetMove(VECTOR3 vTarget, float fSpeedIn, float fRotSpeed)
{
	...
	float fLen, fSpeed;
	const float fNearLimit = 0.5f;    // 目的地との近接リミット(10cm以内に近接したとき)
...
CBaseObj*  CBaseObj::SetKeepOffset(CBaseProc* pProc)
{
	...
	if (pTargetObj) {	// 半径内に入った相手がいた場合
		SetKeepOffsetMove(pTargetObj, fMvMax, vMy);
		// グループ移動への遷移処理
		if (pTargetObj->m_bGroup)
		{
			m_bGroup = TRUE;
		}
	}
...
CBaseObj* CBaseObj::SetKeepOffset(CBaseObj* pOtherObj)
{
	float	fMyRadius, fOtherRadius, fAddRadius, fLen, fMvMax = 0.0f;
	VECTOR3  vMy, vOther;
	VECTOR2  vMy2, vOther2;
	MATRIX4X4	 matWk1, matWk2;
	CBaseObj* pTargetObj = NULL;
	CBBox* pOtherBBox;
	...
	if (!pOtherObj->m_bActive) return NULL;	// 現在生きていなければ対象外
	if (pOtherObj == this) return NULL;	// 自分自身は対象外
	pOtherBBox = pOtherObj->m_pBBox;
	if (pOtherBBox == NULL) return NULL;	// バウンディングボックスがなければ対象外
	vOther = GetPositionVector(pOtherObj->m_mWorld);
	vOther2.x = vOther.x;
	vOther2.y = vOther.z;
	...
	if (pTargetObj) {	// 半径内に入った相手がいた場合
		...
		// 接触しているのがPCならば
		if (pTargetObj->m_nTag == PC) {
			m_bGroup = TRUE;
		}
	}
}

void CBaseObj::SetKeepOffsetMove(CBaseObj* pTargetObj, float fMvMax, VECTOR3 vMy)
{
	VECTOR3 vOther = GetPositionVector(pTargetObj->m_mWorld);
	vOther.y = vMy.y;
	MATRIX4X4 matWk1 = GetLookatMatrix(vMy, vOther);					// 相手の方向を向かせるマトリックスを作成する
	MATRIX4X4 matWk2 = XMMatrixTranslation(0.0f, 0.0f, -fMvMax);		// くい込み値だけ、Ｚ方向の反対方向に移動するマトリックスを作成する
	matWk1 = matWk2 * matWk1;								// くい込み値の反対方向に移動する
	vMy.x = 0.0f - vMy.x;
	vMy.y = 0.0f - vMy.y;
	vMy.z = 0.0f - vMy.z;
